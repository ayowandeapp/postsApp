                            ...
                            <form method="post" @submit.prevent="addPost">
                                <div class="form-row">
                                    <div class="form-group col-md-12">
                                        <label>Title</label>
                                        <input type="text" class="form-control" id="title" placeholder="Post title" v-model="post.title" required>
                                    </div>
                                </div>
...
<script>
    export default {
        ...
        methods:{
        	addPost(){
        		axios.post('/posts/store',{title:this.post.title, body:this.post.body })
        		.then(response=>{
